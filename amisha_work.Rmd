...
plot(bike_ts, main = "Daily BikeShare Trips in Toronto", ylab = "Number of Trips")
...
```{r}
par(mfrow = c(1, 1))
plot(bike_ts, main = "Daily Trips", ylab = "Number of Trips")

# maybe a zoom on a couple of years
plot(window(bike_ts, start = c(2019, 1), end = c(2021, 365)),
     main = "Daily Trips Between 2019–2021", ylab = "Number of Trips")
...
```{r}
acf(as.vector(bike_ts), lag.max = 100,
    main = "ACF of Daily Trips", col = "steelblue")
spec_bike <- spectrum(as.vector(bike_ts), spans = 5)
1 / spec_bike$freq[which.max(spec_bike$spec)]  # estimated period

```
In the ACF plot above, as the lag increases, we see the ACF gradually 
decreasing indicating that there is some sort of trend present in our data.
...
plot(model_data$tim, model_data$y_trans, type = "l", col = "gray",
     main = "Model 0: Monthly Seasonality Only", ylab = "Transformed Trips", xlab = "Time (Year)")
...
plot(model_data$tim, model_data$y_trans, type = "l", col = "gray",
     main = "Model 1: Month + Weekend Indicator", ylab = "Transformed Trips", xlab = "Time (Year)")
...
plot(model_data$tim, model_data$y_trans, type = "l", col = "gray",
     main = "Model 2: Month + Full Weekday (First Year Zoom)", ylab = "Transformed Trips", xlab = "Time (Year)")
...
BIC(mod_base, mod_weekend, mod_weekday)
```
Including both AIC and BIC but focus more on BIC since it has a greater/harsher penalty 
factor for more parameters.
```{r}
...
plot(results_ts_cv$Degree, results_ts_cv$MSE, type = "b", col = "steelblue", pch = 19,
...
##redo for removing seaosnlaity trail 2
```{r}
# --- 1. Refit the Winner (Degree 2) on Full Data ---
final_model2 <- lm(y_trans ~ poly(tim, 2, raw = FALSE) + month + weekday + month*weekday, data = model_data)
...
# Plot B: Q-Q Plot
# Checks for Normality (points should hug the blue line)
car::qqPlot(final_model2$residuals, 
            pch = 16, 
            col = adjustcolor("black", 0.7),
            main = "Normal Q-Q Plot",
            xlab = "Theoretical Quantiles", 
...
# Plot C: Fitted Values vs. Residuals
# Checks for Homoscedasticity (spread should be constant, no funnel shape)
plot(fitted(final_model2), residuals(final_model2), 
     pch = 16, 
     col = adjustcolor("black", 0.4),
     main = "Residuals vs. Fitted Values", 
     xlab = "Fitted Values", 
...
print("--- Shapiro-Wilk Test for Normality ---")
# Limit to 5000 samples because shapiro.test() has a hard limit
if(length(residuals(final_model2)) > 5000) {
  print(shapiro.test(residuals(final_model2)[1:5000]))
} else {
  print(shapiro.test(residuals(final_model2)))
}

print("--- Kolmogorov-Smirnov Test for Normality ---")
print(ks.test(scale(residuals(final_model2)), "pnorm"))

print("--- Runs Test for Randomness ---")
# Checks if residuals have random sign flips
print(randtests::runs.test(residuals(final_model2)))
```
