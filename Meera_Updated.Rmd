...
```{r}
# --- Regularized Regression Analysis (Lasso) ---
library(glmnet)

# 1. Prepare Data Matrices (glmnet requires matrix input, not formula)
# We use the same Training (2017-2022) and Validation (2023) split
# Model Structure: Trend + Month + DayOfWeek + Fourier (Kitchen Sink)

# Create model matrix for Training Data
# -1 removes the intercept column (glmnet handles intercept automatically)
x_train_lasso <- model.matrix(y_trans ~ time_index + month_fac + weekday_fac + sin_year, 
                              data = train_seas)[, -1]
y_train_lasso <- train_seas$y_trans

# Create model matrix for Validation Data
x_valid_lasso <- model.matrix(y_trans ~ time_index + month_fac + weekday_fac + sin_year, 
                              data = valid_seas)[, -1]
y_valid_lasso <- valid_seas$y_trans

# 2. Cross-Validation to find Optimal Lambda
set.seed(443) # For reproducibility
cv_lasso <- cv.glmnet(x_train_lasso, y_train_lasso, alpha = 1) # alpha=1 is Lasso
...
lasso_model <- glmnet(x_train_lasso, y_train_lasso, alpha = 1, lambda = best_lambda)

# 4. Inspect Coefficients
# This shows which variables Lasso kept vs. shrunk to zero
print("Lasso Coefficients:")
print(coef(lasso_model))

# 5. Compare Performance on Validation Set
# OLS Prediction (Using your mod5 from Step 6)
pred_ols <- predict(mod5, newdata = valid_seas)
rmse_ols <- sqrt(mean((y_valid_lasso - pred_ols)^2))

# Lasso Prediction
pred_lasso <- predict(lasso_model, newx = x_valid_lasso)
rmse_lasso <- sqrt(mean((y_valid_lasso - pred_lasso)^2))

# Results Comparison
comparison_table <- data.frame(
  Method = c("OLS Regression", "Lasso Regression"),
  RMSE_Validation = c(rmse_ols, rmse_lasso)
)
print(comparison_table)

# Visual Comparison on Validation Set (Zoomed in)
par(mfrow=c(1,1))
plot(valid_seas$trip_date, y_valid_lasso, type="l", col="gray", lwd=2,
     main="OLS vs Lasso on Validation Set (2023)", ylab="Transformed Trips", xlab="Date")
lines(valid_seas$trip_date, pred_ols, col="blue", lwd=1)
lines(valid_seas$trip_date, pred_lasso, col="red", lty=2, lwd=1) # Dashed line for Lasso
legend("bottomright", legend=c("Actual", "OLS", "Lasso"), 
       col=c("gray", "blue", "red"), lty=c(1, 1, 2))
```
